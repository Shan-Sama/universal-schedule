...
export function groupDisplay(value:string){
  const group=formatGroup(value);
  return group==='CL'?'CL – cả lớp':group;
}
...
export function meetingCompact(meeting:Meeting){
  ...
  return [time,meeting.componentType,group,meeting.room&&`Phòng ${meeting.room}`,`GV: ${meeting.lecturer||'Chưa có'}`].filter(Boolean).join(' · ');
}
